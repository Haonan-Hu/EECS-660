...
def check_prefer_m1_over_m(prefer_list, woman, man, man1):
    ...
    for i in range(n):
        ...
        if prefer_list[woman-1][i] == man:
            index_man = i
    # print(f"{man}:{index_man}, {man1}:{index_man1}")
...
def stable_matching():
    ...
    while True:
        ...
        if matched_women[w-1] == -1:
            ...
            men[m-1] = [-1 if x == w else x for x in men[m-1]]  # mark the woman in m's list as proposed
        # if woman w is already engaged with m1
            ...
            if check_prefer_m1_over_m(women, w, m, m1):
                # mark off the woman that just proposed and man m remain free
